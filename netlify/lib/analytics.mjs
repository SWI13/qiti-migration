 ...
 */
import { listOrdersInRange, algiersDate, getStock, normalizeDzPhone } from './store.mjs';
...
async function computeDashboardSummary(days, now) {
  ...
  const [orders, prevOrders, products, categories, variantStockRows, legacyStock, visitDays, trackingSince, campaigns] =
    await Promise.all([
  ...
  for (const order of orders) {
    ...
    if (isDelivered(order)) {
      revenueByDay.set(placedDay, (revenueByDay.get(placedDay) ?? 0) + (order.total ?? 0));
    } else if (order.status === 'accepted' && !order.deliveryStatus) {
      pipelineByDay.set(placedDay, (pipelineByDay.get(placedDay) ?? 0) + (order.total ?? 0));
    }
  }
  ...
  const campaignRows = campaigns
    ...
    .map((c) => {
      ...
      const cRevenue = cOrders.filter(isDelivered).reduce((sum, o) => sum + (o.total ?? 0), 0);
      return {
        id: c.id,
        name: c.name,
        slug: c.slug,
