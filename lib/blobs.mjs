...
/* الاتصال يتبنى عند أوّل استعمال ماشي عند الاستيراد: سكريبتات التحقّق
   ...
   حتى حاجة */
/*
 * أسماء المتغيّرات تختلف حسب من وين ربطتي التخزين:
 *   Upstash من المتجر  → UPSTASH_REDIS_REST_*
 *   Vercel KV (القديم) → KV_REST_API_*
 * وجوجهم نفس الـ REST API، فنقبلوهم كامل بدل ما نفرضو اسم واحد.
 *
 * ⚠️ REDIS_URL وحدها (redis://…) ماشي كافية: هاذيك وصلة TCP، والعميل
 * هنا يخدم بـ REST فوق HTTPS — الفنكشنات serverless، وصلة TCP دايمة
 * ما عندهاش معنى فيها. إذا كان عندك غير REDIS_URL، لازم تربط تخزين
 * يعطي REST (Upstash) ولا تبدّل العميل.
 */
...
function redis() {
  ...
  if (!url || !token) {
    const hasTcpOnly = Boolean(process.env.REDIS_URL || process.env.KV_URL);
    throw new Error(hasTcpOnly
      ? 'التخزين مربوط بـ REDIS_URL (TCP) برك — هذا العميل يحتاج REST: '
        + 'UPSTASH_REDIS_REST_URL/TOKEN ولا KV_REST_API_URL/TOKEN'
      : 'التخزين ماشي مضبوط: UPSTASH_REDIS_REST_URL و UPSTASH_REDIS_REST_TOKEN ناقصين');
  }
...
function redisStore(name) {
  ...
  return {
    ...
    /*
     * يرجّع `{ modified }` — واش الكتابة صرات فعلاً.
     *
     * ⚠️ `onlyIfNew` كانت تتقرا في catalog.mjs وما كانت تتطبّق هنا: كل
     * نداء كان يكتب فوق الموجود، والجواب كان نص "OK" — والمستدعي
     * يفكّكو بـ `const { modified }` فياخذ undefined، يعني "ما تكتبش
     * شي" على كتابة صرات. النتيجة اللي بانت للمشغّل: كل حفظ منتج من
     * اللوحة كان يبذر صفوف المخزون من جديد ويرجّع الكمية لصفر
     * (seedVariantStock)، وحجز الروابط كان يعتمد على قراية ورا الكتابة
     * بدل ما يكون ذرّي. SET NX يحلّ الزوج.
     */
    async setJSON(key, value, opts) {
      const args = {};
      if (opts?.ttlSeconds) args.ex = opts.ttlSeconds;
      if (opts?.onlyIfNew) args.nx = true;

      const written = await redis().set(
        full(key), JSON.stringify(value), Object.keys(args).length ? args : undefined,
      );
      /* بلا nx الكتابة تصرا ديما؛ مع nx، Redis يرجّع null إذا المفتاح كان موجود */
      return { modified: opts?.onlyIfNew ? written !== null : true };
    },
    ...
    /*
     * زيادة وحدة، والقيمة الجديدة ترجع من السيرفر.
     *
     * ⚠️ الفرق مع hincr فوق ماشي شكلي: العدّاد اللي يزيد وما يرجّعش
     * القيمة يلزمو hget من بعد، وبين الزوج تقدر تدخل زيادة أخرى —
     * فزوج منتجات يتصنعو في نفس اللحظة ياخذو نفس الرقم التسلسلي.
     * HINCRBY يرجّع القيمة اللي هو كتبها، فكل نداء ياخذ رقم وحدو.
     */
    async hincrBy(key, field, by = 1) {
      return Number(await redis().hincrby(full(key), field, by));
    },

    /*
     * عدّاد يزيد ويموت وحدو — أساس تحديد المعدّل (lib/rate-limit.mjs).
     *
     * ⚠️ الزوج في pipeline وحدة بقصد: INCR وحدها تخلّي المفتاح يعيش
     * للأبد، وEXPIRE في نداء ثاني تقدر ما توصلش (الفنكشن تموت بيناتهم)
     * فيبقى مفتاح خالد على كل IP عدّى من الموقع. الرحلة وحدة، والنتيجة
     * إمّا الزوج ولا حتى واحد.
     */
    ...
    /* ── قوائم (سجلّ الأحداث) ──────────────────────────────────────
     *
     * ⚠️ علاش قائمة وماشي مفتاح لكل حدث: سجلّ التدقيق يتكتب بزّاف
     * ويتقرا بالصفحات ("آخر 50"). مفتاح لكل حدث معناه SCAN على آلاف
     * المفاتيح في كل قراية — وهذا يبطّي اللوحة كامل. LPUSH يزيد في
     * الرأس بتكلفة ثابتة، وLRANGE يقرا نافذة وحدة برك.
     *
     * `cap` يقصّ الذيل في نفس الرحلة (LTRIM): السجلّ يبقى محدود بلا
     * ما نحتاجو مهمّة تنظيف تجري وحدها وتنسى.
     */
    async pushCapped(key, value, cap) {
      const client = redis();
      const k = full(key);
      const pipe = client.pipeline();
      pipe.lpush(k, typeof value === 'string' ? value : JSON.stringify(value));
      if (cap > 0) pipe.ltrim(k, 0, cap - 1);
      const [length] = await pipe.exec();
      return Number(length) || 0;
    },

    /** نافذة من القائمة، من الأحدث (0) للأقدم. `stop` شامل كيما Redis. */
    async range(key, start, stop) {
      const rows = await redis().lrange(full(key), start, stop);
      return (rows ?? []).map((row) => {
    ...
    async length(key) {
      return Number(await redis().llen(full(key))) || 0;
    },
  };
}
...
function blobStore(name) {
  ...
  return {
    ...
    /* نفس شكل الجواب تاع مخزن Redis — المستدعي ما يلزموش يعرف وين
       راه يكتب. المخزن هذا للصور برك، فما كاينش onlyIfNew فيه. */
