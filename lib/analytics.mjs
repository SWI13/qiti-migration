 ...
 */
import { listOrdersInRange, algiersDate, getStock, getCosts, normalizeDzPhone, countsInBooks } from './store.mjs';
 ...
 * يرمي الكاش.
 *
 * ⚠️ لازم بعد أي حاجة تبدّل الأرقام من اللوحة روحها — مزامنة الطرود،
 * إلغاء طلب من الدفاتر. بلاه، تنقر "زامن"، الطرود تتحدّث فعلاً،
 * والصفحة توري نفس الأرقام القدام لمدّة دقيقة — فتنقر مرّة أخرى،
 * وتحسب الزرّ ما يخدمش.
 */
...
async function computeDashboardSummary(days, now) {
  ...
  /* الملغى من الدفاتر يخرج من هنا، من أوّل سطر — بلا هذا لازم نتفكّرو
     نفلترو في كل حساب تحت (مداخيل، ربح، وحدات، تحويل، أفضل منتج...)
     وواحد منهم ينسى */
  const [orders, prevOrders, products, categories, variantStockRows, legacyStock, visitDays, trackingSince, campaigns, costs] =
    await Promise.all([
      listOrdersInRange(startDay, endDay).then((rows) => rows.filter(countsInBooks)),
      listOrdersInRange(prevStartDay, prevEndDay).then((rows) => rows.filter(countsInBooks)),
      listProducts(),
      listCategories(),
      listAllVariantStock(),
      getStock(),
