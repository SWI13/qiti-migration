...
import { getSettings, notifies } from '../settings.mjs';
 ...
 */
export async function sendShipment(orderId, { by = null, force = false } = {}) {
  ...
  if (!check.ok) {
    const failed = await markFailed(order, check.error, attempts, by);
  ...
  if (!tracking) {
    ...
    if (created.error) {
      const failed = await markFailed(order, created.error, attempts, by, { retryAfter: created.retryAfter });
      return { ...fail(created.error), order: failed };
    }

    tracking = created.data?.tracking ?? created.data?.data?.tracking ?? null;
    if (!tracking) {
      const failed = await markFailed(order, 'الموصّل لم يرجع رقم تتبّع.', attempts, by);
  ...
  if (valid.error) {
    ...
    const failed = await markFailed(order, valid.error, attempts, by, { tracking });
 ...
 */
export async function cancelShipment(orderId, { by = null } = {}) {
  const order = await getOrder(orderId).catch(() => null);
...
/** واش نبعثو الطردة وحدها كي يتقبّل الطلب */
export async function autoShipEnabled() {
  const settings = await getSettings().catch(() => null);
  return settings?.autoShip !== false;
}
