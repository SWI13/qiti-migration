...
/*
 * 0661445566 → +213661445566 (E.164 مع '+' — هذي صيغة تيك توك).
 *
 * كاين وحدة كيفها في message.mjs (toE164Dz) وما نستعملوهاش هنا بقصد:
 * هذا الملف يتستورد من settings.mjs، وmessage.mjs يستورد settings.mjs
 * — الاستيراد يولّي دايري في حلقة. وزيد، meta.mjs عندو نسختو هو
 * (phoneForMeta، بلا '+'): كل منصّة وصيغتها، والقرب بيناتهم يخدع.
 */
const phoneForTikTok = (localPhone) =>
...
/**
 * أشمن بيكسل يخدم لهذه الصفحة.
 *
 * الترتيب: بيكسل الصفحة (الحملة) → بيكسل الصفحة الرئيسية (الإعدادات،
 * يتبدّل من اللوحة) → المتغيّر/الافتراضي المكتوب في الكود.
 *
 * علاش كل صفحة تستاهل بيكسل وحدها: كل صفحة هبوط تقدر تكون منتج آخر،
 * وحتى حساب إعلاني آخر. بيكسل واحد لكلش يخلط الجماهير والتحويلات تاع
 * منتجين في نفس القمع — والخوارزمية تتعلّم على خليط ماشي على منتج.
 */
export const resolvePixelId = ({ campaign = null, settings = null } = {}) =>
  sanitizePixelId(campaign?.tiktokPixelId)
  || sanitizePixelId(settings?.tiktokPixelMain)
  || tiktokPixelId();
...
export function tiktokPixelSnippet(rawId = tiktokPixelId()) {
  const body = pixelScriptBody(rawId);
  return body ? `<!-- TikTok Pixel -->\n<script>\n${body}\n</script>` : '';
}

/**
 * نفس الكود، بلا وسم `<script>` — باش يتقدّم كملف جافاسكريبت.
 *
 * علاش موجود: الصفحة الرئيسية ملف ستاتيك، والـ id تاعها يتبدّل من
 * اللوحة. لو حقنّاه في البناء، كل تبديل يلزمو نشر جديد — واللوحة
 * تولّي تكذب على المشغّل. الصفحة تنادي /api/render?pixel=1، والجواب
 * يتخبّى في الـ CDN خمس دقايق كيما الصفحات المعروضة.
 */
export function pixelScriptBody(rawId = tiktokPixelId()) {
...
/* الرابط اللي تنادي منّو الصفحة الستاتيك — نفس الفنكشن تاع العرض،
   ماشي فنكشن جديدة: حصّة الفنكشنات في Vercel محدودة (كيما البلديات
   في api/render.mjs) */
export const PIXEL_SCRIPT_URL = '/api/render?pixel=1';

/**
 * الوسم اللي يتحقن في index.html. ديما يتحقن، حتى لو البيكسل مطفي —
 * الـ id يتقرّر وقت الطلب ماشي وقت البناء، والجواب يخرج فارغ إذا كان
 * مطفي.
 */
export const tiktokPixelLoaderTag = () =>
  `<!-- TikTok Pixel --><script src="${PIXEL_SCRIPT_URL}" defer></script>`;
...
function userData(order) {
  ...
  if (order.phone) {
    const e164 = phoneForTikTok(order.phone);
