...
/**
 * واش هاذ الطلب يتحسب في الدفاتر.
 *
 * ⚠️ فنكشن وحدة يستعملوها اللوحة وتقرير اليوم وتقرير الأسبوع. الفلترة
 * اللي تتكتب ثلاث مرّات تنسى وحدة، وساعتها التقرير يقول رقم واللوحة
 * تقول رقم آخر على نفس اليوم — وهذا يخلّي التاجر ما يثق في حتى واحد.
 */
export const countsInBooks = (order) => !order?.voidedAt;

export async function saveOrder(record) {
  await orders().setJSON(record.id, record);
  await syncOpenIndex(record);
  await indexCustomerOrder(record);
  return record;
}
...
async function syncOpenIndex(record) {
  if (!record?.id) return;
  try {
    /* الملغى من الدفاتر يخرج من الصفّ تاني — "انساه" تعني انساه، ماشي
       "انساه في الأرقام وخلّيه يعيّطلك" */
    if (record.status === 'pending' && countsInBooks(record)) await openIndex().set(record.id, '1');
...
export async function rebuildOpenIndex() {
  const pending = (await listAllOrders())
    .filter((order) => order.status === 'pending' && countsInBooks(order));
...
export async function listPendingOrders() {
  ...
  records.forEach((order, position) => {
    if (order?.status === 'pending' && countsInBooks(order)) pending.push(order);
...
/* ── فهرس الزبون ──────────────────────────────────────────────────
 *
 * ⚠️ علاش كاين: `listOrdersByPhone` كانت تمسح الأرشيف كامل، وهي
 * تتنادى في **كل طلب جديد** (لقطة تاريخ الزبون في api/order.mjs).
 * يعني الزبونة اللي تنقر "أكّد" تستنّى قراية كل طلب صرا في تاريخ
 * المحل. على ألف طلب هذا ألف قراية على الطريق الحرج تاع البيع — نفس
 * العطب اللي تصلّح في صفّ المكالمات بفهرس الطلبات المفتوحة، بصح
 * هذا في بلاصة أخطر: الزبونة راهي تستنّى.
 *
 * الفهرس: رقم الهاتف (موحّد) → لائحة ids. يتحدّث في saveOrder روحها.
 */
const CUSTOMER_INDEX = 'customer-orders';
const customerIndex = () => getStore(CUSTOMER_INDEX);
const CUSTOMER_BUILT_KEY = '_built';
/* نحتفظو بآخر 50 طلب للزبون — التاريخ يتحسب على "شحال وصلت / رفض /
   رجعت"، و50 طلب من زبون واحد أكثر من كافية باش تعرف مع من تتعامل */
const CUSTOMER_HISTORY_LIMIT = 50;

/* المفتاح موحّد: +213661445566 و0661445566 نفس الزبون. الرقم اللي ما
   يتوحّدش (بيانات قديمة غالطة) ياخذ مفتاحو الخام بدل ما يضيع. */
const customerKey = (phone) => normalizeDzPhone(phone) ?? String(phone ?? '').trim();

async function indexCustomerOrder(record) {
  const key = customerKey(record?.phone);
  if (!key || !record?.id) return;
  try {
    const existing = (await customerIndex().get(key, { type: 'json' })) ?? [];
    if (existing.includes(record.id)) return;
    const next = [...existing, record.id].slice(-CUSTOMER_HISTORY_LIMIT);
    await customerIndex().setJSON(key, next);
  } catch (error) {
    /* نفس قاعدة فهرس الطلبات المفتوحة: الفهرس تسريع ماشي حقيقة.
       فشلو ما يقدرش يطيّح تسجيل طلب — القراية ترجع للمسح الكامل. */
    console.error('Customer index sync failed:', error.message, '| order:', record.id);
  }
}

/** يعاود يبني الفهرس من الأرشيف كامل — مرّة وحدة، أوّل قراية بعد النشر */
async function rebuildCustomerIndex() {
  const all = await listAllOrders();
  const byPhone = new Map();
  for (const order of all.sort((a, b) => (a.createdAt ?? '').localeCompare(b.createdAt ?? ''))) {
    const key = customerKey(order.phone);
    if (!key) continue;
    byPhone.set(key, [...(byPhone.get(key) ?? []), order.id].slice(-CUSTOMER_HISTORY_LIMIT));
  }

  const index = customerIndex();
  await Promise.all([...byPhone].map(([key, ids]) => index.setJSON(key, ids)));
  await index.set(CUSTOMER_BUILT_KEY, new Date().toISOString());
 ...
 *
 * الطلب اللي في الفهرس وما بقاش موجود (تمسح بـ /clear وسط الطريق)
 * يتصفّى هنا — نفس منطق listPendingOrders.
 */
export async function listOrdersByPhone(phone) {
  ...
  const index = customerIndex();
  const built = await index.get(CUSTOMER_BUILT_KEY).catch(() => null);

  if (!built) {
    const { all, byPhone } = await rebuildCustomerIndex().catch(() => ({ all: null, byPhone: null }));
    /* البناء طاح؟ نرجعو للمسح الكامل — الجواب أهمّ من التسريع */
    if (!all) return (await listAllOrders()).filter((order) => customerKey(order.phone) === key);
    const wanted = new Set(byPhone.get(key) ?? []);
    return all.filter((order) => wanted.has(order.id));
  }

  const ids = (await index.get(key, { type: 'json' }).catch(() => null)) ?? [];
  if (!ids.length) return [];

  const records = await Promise.all(ids.map((id) => getOrder(id).catch(() => null)));
  return records.filter(Boolean);
}
...
/* ── محو طلب واحد (ولا طلبات زبون) نهائياً ───────────────────────
 *
 * ⚠️ هذا ماشي `voidOrder`. الإلغاء يخلّي السجلّ ويخرّجو من الحساب برك
 * — يخدم كي تحب تعرف علاش الرقم تبدّل. المحو هنا يحيّد الطلب من
 * التخزين كامل: ما يبقى منّو والو، لا في اللوحة لا في التقارير لا في
 * تاريخ الزبون. يخدم للطلبات التجريبية اللي ما لازمش يعيشو أصلاً.
 *
 * ⚠️ المخزون ما يتمسّش — نفس قاعدة الإلغاء والرجعة: الفلوس والمخزون
 * يتحرّكو كل واحد بأمرو. الطلب اللي خرج فعلاً، السلعة برّا؛ واللي كان
 * تجريبي ترجّع كميتو بـ /restock.
 *
 * ⚠️ الفهارس تتنظّف هنا بيدها. الفهرس اللي يشير لطلب ممسوح يخلّي صفّ
 * المكالمات يوري سطر فارغ، وتاريخ الزبون يوري طلب ما بقاش موجود.
 */

/** يمسح رسائل "علاش رفضتو؟" اللي كانو مربوطين بطلب ممسوح */
async function forgetReplyPromptsForOrder(orderId) {
  const store = replies();
  const { blobs } = await store.list();
  const owned = await Promise.all(blobs.map(async (blob) => {
    const value = await store.get(blob.key).catch(() => null);
    return value === orderId ? blob.key : null;
  }));
  await Promise.all(owned.filter(Boolean).map((key) => store.delete(key)));
}

/** يحيّد id الطلب من تاريخ الزبون، ويمسح المفتاح كي يبقى فارغ */
async function unindexCustomerOrder(record) {
  const key = customerKey(record?.phone);
  if (!key || !record?.id) return;
  const index = customerIndex();
  const existing = (await index.get(key, { type: 'json' }).catch(() => null)) ?? [];
  const next = existing.filter((id) => id !== record.id);
  if (next.length) await index.setJSON(key, next);
  else await index.delete(key);
}

/** يمسح طلب واحد وكل ما يشير ليه. يرجع الطلب الممسوح، ولا null إذا ما كانش. */
export async function deleteOrder(id) {
  const record = await getOrder(id).catch(() => null);
  if (!record) return null;

  await orders().delete(id);
  await openIndex().delete(id).catch((error) =>
    console.error('Open-order index cleanup failed:', error.message, '| order:', id));
  await unindexCustomerOrder(record).catch((error) =>
    console.error('Customer index cleanup failed:', error.message, '| order:', id));
  await forgetReplyPromptsForOrder(id).catch((error) =>
    console.error('Reply-prompt cleanup failed:', error.message, '| order:', id));

  return record;
}

/**
 * يمسح كل طلبات رقم واحد — الرقم يتوحّد قبل، فـ 0661445566 و+213661445566
 * نفس الزبون (شوف normalizeDzPhone).
 *
 * يرجع { phone, deleted } — `deleted` هي الطلبات كيما كانو قبل المحو،
 * باش اللي ينادي يقدر يقول للمشغّل واش راح بالضبط.
 */
export async function deleteOrdersByPhone(phone) {
  const key = customerKey(phone);
  if (!key) return { phone: null, deleted: [] };

  /* الفهرس يقدر يكون قديم — نصفّيو بالرقم روحو باش ما نمسحوش طلب زبون آخر */
  const mine = (await listOrdersByPhone(key)).filter((order) => customerKey(order.phone) === key);

  const deleted = [];
  for (const order of mine) {
    const gone = await deleteOrder(order.id);
    if (gone) deleted.push(gone);
  }

  /* المفتاح روحو يمشي — unindexCustomerOrder تمسحو كي يخلا، وهذا احتياط
     كي يكون الفهرس فيه ids تاع طلبات ما بقاوش موجودين */
  await customerIndex().delete(key).catch(() => {});

  return { phone: key, deleted };
}
...
export async function clearAllOrders() {
  ...
  /* الفهارس تمشي معاهم — فهرس يشير لطلبات ما بقاوش يخلّي الصفّ يوري
     سطور فارغة حتى تجي أوّل قراءة تنظّف، وفهرس الزبون يخلّي تاريخ
     زبون يبان وهو راه تمسح */
  for (const index of [openIndex(), customerIndex()]) {
