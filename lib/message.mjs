 ...
 */
/*
 * ⚠️ `COURIER_COST` تحيّدت — كانت 350 ثابتة وما بقاش عندها حتى قارئ.
 * تكلفة التوصيل ولّات تتبدّل بـ `/cost courier` (صفر بالتلقائي: في
 * الدفع عند الاستلام الزبونة هي اللي تخلّص)، وسومة التوصيل اللي تبان
 * في الصفحة تجي من `shipping-rates.mjs` حسب الولاية. ثابتة ميتة جنب
 * قيمة حيّة تعطي نفس الاسم لرقمين مختلفين — وهذا كيفاش يجي نهار
 * وتتحسب الغالطة.
 */
  ...
  if (order.deliveryStatus === 'returned') {
    ...
    if (c.returnRules) {
      return -returnLossFor({
        ...
        /* الإعلان تخلّص باش يجي هاذ الطلب — يرجع ولا ما يرجعش، الفلوس
           راحت. شوف returnLossFor في settings.mjs. */
        adsCost: c.adsCost ?? 0,
      }, {
...
/*
 * عنوان الرسالة — يتبع حالة الطلب، ماشي ثابت.
 *
 * ⚠️ الرسالة وحدة وتتعاود ترسم في بلاصتها على كل تبديل. كان العنوان
 * مكتوب "🐱 طلب جديد" ديما — فطلب وصل من ثلاث أيام يبقى مكتوب فوقو
 * "طلب جديد"، والحالة الحقيقية مدفونة عشر أسطر تحت. في گروب فيه
 * خمسين رسالة، السطر الأوّل هو الوحيد اللي يتقرا كي تعمل scroll.
 *
 * والقطّة راحت معاه: كانت من الطوق كي كان هو المنتج الوحيد. دروك
 * Qiti تبيع أي حاجة، وقطّة فوق طلبية غطاء تيليفون تقرا كغلطة.
 */
export function orderHeadline(record) {
  /* الملغى من الدفاتر يبان أوّل حاجة: الرسالة تبقى تقول "وصل" تحت،
     وبلا هاذ السطر تقرا الرسالة وتحسبها بيعة */
  if (record?.voidedAt) return '🚫 <b>خارج الحساب</b>';
  if (record?.status === 'denied') return '❌ <b>طلب مرفوض</b>';
  if (record?.status === 'accepted') {
    if (record.deliveryStatus === 'delivered') return '✅ <b>طلب وصل</b>';
    if (record.deliveryStatus === 'returned') {
      return record.returnReceivedAt ? '📥 <b>رجعة استُلمت</b>' : '↩️ <b>طلب رجع</b>';
    }
    const stage = record.shipment?.stage;
    if (stage === 'out_for_delivery') return '🚚 <b>خرجت للتوصيل</b>';
    if (stage === 'in_transit' || stage === 'picked') return '📦 <b>في الطريق</b>';
    if (record.shipment?.tracking) return '📦 <b>طلب مشحون</b>';
    return '✅ <b>طلب مقبول</b>';
  }
  if (record?.confirmedAt) return '📞 <b>طلب مؤكَّد — ينتظر القرار</b>';
  return '🛒 <b>طلب جديد</b>';
}

/*
 * الطردة داخل رسالة الطلب.
 *
 * ⚠️ رقم التتبّع كان يبان في إشعار منفصل وقت ما تتخلق الطردة برك —
 * ومن بعد يمشي مع الـ scroll. المشغّل اللي يحبّ يعرف "وين راهي
 * طردة هاذ الطلب" كان يلزمو يقلّب في الگروب ولا يفتح اللوحة، والرسالة
 * تاع الطلب — اللي هي المرجع تاعو — تسكت على الجواب.
 *
 * الغلطة تبان هنا تاني: الطردة اللي ما خرجتش ما تخلّي الطلب يبان
 * عادي، وإلا تلقى روحك مستنّي موصّل ما جاه والو.
 */
export function shipmentLines(record) {
  const shipment = record?.shipment;
  if (!shipment?.tracking && !shipment?.lastError) return [];

  const out = [''];

  if (shipment.tracking) {
    out.push(`🚚 <b>الطردة:</b> <code>${esc(shipment.tracking)}</code>`);
    const stage = SHIPMENT_STAGE_LABEL[shipment.stage];
    if (stage) out.push(`📍 ${esc(stage)}`);
  }

  if (shipment.state === 'failed' && shipment.lastError) {
    out.push(`⚠️ <b>الطردة ما خرجتش:</b> ${esc(String(shipment.lastError).slice(0, 160))}`);
    out.push(`أعد المحاولة: <code>/ship ${esc(record.id)}</code>`);
  }
...
/*
 * نسخة من التسميات تاع ecotrack/status.mjs.
 *
 * ⚠️ نسخة بقصد: message.mjs يتحمّل في كل نداء تقريباً، وستيراد
 * طبقة الموصّل كاملة باش نجيبو عشر كلمات يجرّ معاه العميل وgeo
 * والإعدادات. النسخة صغيرة وثابتة، والفحص في verify يمنعها من
 * تنحرف على الأصل.
 */
export const SHIPMENT_STAGE_LABEL = {
  created: 'تخلقت',
...
export function ownerMessage(record) {
  const lines = [`${orderHeadline(record)} — Qiti`];
  ...
  if (record.voidedAt) {
    lines.push(
      '',
      `🚫 <b>خارج الحساب</b> — ${esc(record.voidedBy ?? '')} · ${dzTime(new Date(record.voidedAt))}`,
      ...(record.voidReason ? [`💬 ${esc(record.voidReason)}`] : []),
      'ما يتحسبش في المداخيل ولا في الربح.',
    );
  }
  ...
  if (record.status === 'accepted') {
    ...
    lines.push(...shipmentLines(record));
