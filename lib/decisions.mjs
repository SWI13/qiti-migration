...
async function confirmOrderNow(orderId, { by, chatId } = {}) {
...
async function acceptOrderNow(orderId, { by, chatId } = {}) {
  const order = await getOrder(orderId).catch(() => null);
  if (!order) return fail('الطلب غير موجود.');
...
async function denyOrderNow(orderId, { by, reason, chatId } = {}) {
...
async function setDeliveryOutcomeNow(orderId, deliveryStatus, { by, chatId, source = null } = {}) {
...
async function voidOrderNow(orderId, { by, reason, chatId } = {}) {
  const order = await getOrder(orderId).catch(() => null);
  if (!order) return fail('الطلب غير موجود.');
...
async function unvoidOrderNow(orderId, { chatId } = {}) {
  const order = await getOrder(orderId).catch(() => null);
  if (!order) return fail('الطلب غير موجود.');
...
async function receiveReturnNow(orderId, { by, chatId } = {}) {
  const order = await getOrder(orderId).catch(() => null);
  if (!order) return fail('الطلب غير موجود.');
...
async function purgeOrderNow(orderId, { by } = {}) {
  const order = await getOrder(orderId).catch(() => null);
  if (!order) return fail('الطلب غير موجود.');
  ...
  if (order.shipment?.tracking) {
    if (isFinalStage(order.shipment.stage)) {
      shipment = 'final';
    } else {
      const result = await cancelShipment(orderId, { by })
        .catch((error) => ({ ok: false, error: error.message }));
      if (!result.ok) return fail(`الطردة ما تلغاتش: ${result.error} — صلّحها قبل المحو.`);
 ...
 */
export async function purgeOrdersByPhone(phone, { by } = {}) {
  ...
  for (const order of found) {
    const result = await purgeOrder(order.id, { by });
