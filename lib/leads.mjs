...
import { appendCall } from './calls.mjs';
...
/**
 * يسجّل محاولة مكالمة على lead — نفس سجلّ المحاولات تاع الطلبات
 * (lib/calls.mjs)، باش صفّ المكالمات في اللوحة يعالج الزوج بنفس الطريقة.
 *
 * ⚠️ فرق واحد: الـ lead ما عندوش تأكيد. "جاوب" على طلب معناها الزبون
 * أكّد شراء موجود؛ "جاوب" على lead معناها برك كلّمتو — علاش النتيجة
 * تتكتب في contactedAt، ماشي في confirmedAt (اللي يدخل في إحصائيات
 * الطلبات المؤكّدة ويكذّبها).
 */
export async function logLeadCall(phone, input) {
  const lead = await getLead(phone);
  if (!lead) return null;

  const patch = appendCall(lead, input);
  if (!patch) return null;

  const { confirmedAt, confirmedBy, ...rest } = patch;
  return updateLead(phone, confirmedAt
    ? { ...rest, contactedAt: confirmedAt, contactedBy: confirmedBy ?? null }
    : rest);
}
