 ...
 *
 * 3. الإشعار يوصل دروك — ورسالة وحدة تتحدّث في بلاصتها.
 *    كيما الطلب بالضبط: توصلك رسالة ساعة ما الرقم يولّي صحيح. ومن بعد،
 *    كل ما يعمّر الزبون حقل جديد، **نفس الرسالة** تتبدّل (ماشي وحدة
 *    جديدة) — تشوف الطلب يتكوّن قدّامك بلا ما يتعمّر الگروب. وكي
 *    يكمّل، الرسالة روحها تولّي "✅ كمّل الطلب".
 */
 ...
 *
 * الإشعار يتبعث فوراً من api/lead.mjs. هذا الفنكشن يخدم للكنس برك —
 * الـ leads اللي فشل الإشعار تاعهم (تيليغرام طاح، الشبكة قطعت) يعاودو
 * يتجرّبو. مفتوح + عمرو ما تبعث = يستاهل محاولة.
 */
export const dueForNotice = (lead) =>
  Boolean(lead) && lead.status === 'open' && !lead.notifiedAt;
...
export async function sweepLeads() {
  ...
  try {
    ...
    const due = open.filter(dueForNotice);
