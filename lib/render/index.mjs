 ...
 */
/*
 * stock = صفوف مخزون المنتج (من listStockFor). القسم الوحيد اللي
 * يستعملها دروك هو الطلب — باش يقول "باقي 4" كي يكون الرقم الحقيقي
 * قليل. اختيارية: بلاها ما تبان حتى شارة، ماشي تطيح الصفحة.
 *
 * offerProducts = خريطة { id: product } للمنتجات اللي تجي في الباقات
 * وفي العرض الإضافي — القسم يحتاجها باش يكتب أسماءهم وسومهم. بلاها
 * الباقات تتعرض بأسمائها هي برك، ما تطيحش الصفحة.
 */
export function renderSections(campaign, product, { preview = false, stock = [], offerProducts = {} } = {}) {
  ...
  return (campaign?.sections ?? [])
    ...
    .map((section) => {
      ...
      try {
        return render({ data: section.data ?? {}, product, campaign, theme, priceView, preview, stock, offerProducts });
      } catch (error) {
