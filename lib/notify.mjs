...
async function send(text, { event = null, chatId = null } = {}) {
  ...
  try {
    ...
    /*
     * ⚠️ الإشعار اللي ما وصلش يبقى صامت بطبيعتو: ما كاين حتى واحد
     * يستنّى رسالة معيّنة باش يلاحظ نقصانها. السجلّ هو البلاصة
     * الوحيدة اللي تقدر تقول "الإشعار هذا تبعث ومات".
     */
    await logEvent({
      ...
      entityId: event,
      description: `إشعار ${event ?? 'بلا اسم'} ما تبعثش`,
...
/**
 * الطردة تلغات — بيدك من اللوحة ولا بـ /cancel.
 *
 * الفرق بين "تمسحت" و"طلبنا رجعة" مهمّ: قبل التثبيت الطردة تختفي بلا
 * فلوس، وبعدو الموصّل راه يعرف بيها وترجع كطردة رجعة تتخلّص.
 */
export const shipmentCancelled = (order) => send([
  '🚫 <b>الطردة تلغات</b>',
  '',
  `الطلب: ${orderLabel(order)} — ${esc(order.name)}`,
  `التتبّع: <code>${esc(order.shipment?.tracking ?? '')}</code>`,
  `الحالة: ${esc(STAGE_LABEL[order.shipment?.stage] ?? order.shipment?.stage ?? '')}`,
  '',
  order.shipment?.stage === 'return_asked'
    ? '⚠️ الطردة كانت خرجت — الموصّل يرجّعها، والرجعة تتخلّص.'
    : 'تمسحت قبل ما تخرج — بلا تكلفة.',
  'المخزون ما رجعش وحدو: ارفض الطلب ولا سجّل الرجعة باش يرجع.',
...
export async function shipmentReturned(order, costs, settings) {
  ...
  return send([
    ...
    /* الإعلان يبان في السطر: بلاه المجموع تحت ما يتفقش مع اللي فوقو،
       والمشغّل يحسب فيه غلطة */
    ...(breakdown.ads ? [`إعلانات: ${dz(breakdown.ads)}`] : []),
